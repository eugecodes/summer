    ...
    for ii in range(len(tableDescriptionList[i])):
        ...
        if tableDataTypeList[i][ii] == 'long' or tableDataTypeList[i][ii] == 'double':
            ...
            for x in range(0,random_limit):
                random_list.append(randint(-9000000000,(x*-1)))
            ...
            if campo_random_del_mismo_tipo != 999:
                ...
                output_file.write(query)
                query = select + tableNameList[i] + '.' + tableDescriptionList[i][ii] + where + tableNameList[i] + '.' + tableDescriptionList[i][ii] + all_multiply + str(randint(-980000000,-1)) + between + '(' + str(random_list).strip('[]') + ')' + '\n \n'
                output_file.write(query)
